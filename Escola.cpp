...
void main(void)
{
	...
	while (true)
	{
		...
		case LISTAR_PERIODO:
			...
			if (BuscarPeriodo(&nPeriodo, QTDE_PERIODOS, &cOpcao)) {
				LIMPAR_TELA;
				cout << "\tListagem de raias do periodo: ";
				if (nPeriodo == 1) {
					i = 0;
					j = 9;
					cout << "Manha";
				...
				cout << "\nRaia       Sobrenome      Mensalidade\n";
				for (i; i <= j; i++) {
					sprintf_s(cWork, sizeof(cWork), " %3i %12s        %7.2f ", i + 1, vetSobrenomes[i], vetMensalidades[i]);
					cout << cWork << endl;
				}
				PAUSA;
